...
//				return x;
		...
		public static VoronoiGraph FilterVG(VoronoiGraph VG, double minLeftRightDist,double maxLeftRightDist)
		{
			VoronoiGraph VGErg = new VoronoiGraph();
			foreach(VoronoiEdge VE in VG.Edges)
			{
				if(Math.Sqrt(Vector.Dist(VE.LeftData,VE.RightData))>=minLeftRightDist && Math.Sqrt(Vector.Dist(VE.LeftData, VE.RightData)) <= maxLeftRightDist)
					VGErg.Edges.Add(VE);
			}
